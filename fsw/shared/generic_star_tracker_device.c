...
*/
int32_t GENERIC_STAR_TRACKER_ReadData(uart_info_t *device, uint8_t *read_data,
                                      uint8_t data_length) {
  ...
  while ((bytes_available < data_length) &&
         (ms_timeout_counter < GENERIC_STAR_TRACKER_CFG_MS_TIMEOUT)) {
    ms_timeout_counter++;
    OS_TaskDelay(1);
    bytes_available = uart_bytes_available(device);
  }
  ...
  if (ms_timeout_counter < GENERIC_STAR_TRACKER_CFG_MS_TIMEOUT) {
    ...
    if (bytes != bytes_available) {
      ...
      OS_printf(
          "  GENERIC_STAR_TRACKER_ReadData: Bytes read != to requested! \n");
#endif
      status = OS_ERROR;
    } /* uart_read */
  } else {
...
*/
int32_t GENERIC_STAR_TRACKER_CommandDevice(uart_info_t *device,
                                           uint8_t cmd_code, uint32_t payload) {
  int32_t status = OS_SUCCESS;
  int32_t bytes = 0;
  ...
  if (status == UART_SUCCESS) {
    /* Write data */
    bytes = uart_write_port(device, write_data,
    ...
    if (bytes == GENERIC_STAR_TRACKER_DEVICE_CMD_SIZE) {
      status = GENERIC_STAR_TRACKER_ReadData(
          device, read_data, GENERIC_STAR_TRACKER_DEVICE_CMD_SIZE);
      if (status == OS_SUCCESS) {
        /* Confirm echoed response */
        bytes = 0;
        while ((bytes < (int32_t)GENERIC_STAR_TRACKER_DEVICE_CMD_SIZE) &&
               (status == OS_SUCCESS)) {
...
#ifdef GENERIC_STAR_TRACKER_CFG_DEBUG
        OS_printf("GENERIC_STAR_TRACKER_CommandDevice - "
...
#ifdef GENERIC_STAR_TRACKER_CFG_DEBUG
      OS_printf("GENERIC_STAR_TRACKER_CommandDevice - uart_write_port returned "
                "%d, expected %d \n",
...
*/
int32_t
GENERIC_STAR_TRACKER_RequestHK(uart_info_t *device,
                               GENERIC_STAR_TRACKER_Device_HK_tlm_t *data) {
  int32_t status = OS_SUCCESS;
  uint8_t read_data[GENERIC_STAR_TRACKER_DEVICE_HK_SIZE];

  /* Command device to send HK */
  status = GENERIC_STAR_TRACKER_CommandDevice(
      device, GENERIC_STAR_TRACKER_DEVICE_REQ_HK_CMD, 0);
  if (status == OS_SUCCESS) {
    /* Read HK data */
    status =
        GENERIC_STAR_TRACKER_ReadData(device, read_data, sizeof(read_data));
    if (status == OS_SUCCESS) {
#ifdef GENERIC_STAR_TRACKER_CFG_DEBUG
...
#ifdef GENERIC_STAR_TRACKER_CFG_DEBUG
        OS_printf("  GENERIC_STAR_TRACKER_RequestHK: "
                  "GENERIC_STAR_TRACKER_ReadData reported error %d \n",
                  status);
#endif
        status = OS_ERROR;
      }
...
#ifdef GENERIC_STAR_TRACKER_CFG_DEBUG
    OS_printf("  GENERIC_STAR_TRACKER_RequestHK: "
              "GENERIC_STAR_TRACKER_CommandDevice reported error %d \n",
              status);
#endif
  }
  return status;
...
*/
int32_t
GENERIC_STAR_TRACKER_RequestData(uart_info_t *device,
  ...
  /* Command device to send HK */
  status = GENERIC_STAR_TRACKER_CommandDevice(
      device, GENERIC_STAR_TRACKER_DEVICE_REQ_DATA_CMD, 0);
  if (status == OS_SUCCESS) {
    /* Read HK data */
    status =
        GENERIC_STAR_TRACKER_ReadData(device, read_data, sizeof(read_data));
