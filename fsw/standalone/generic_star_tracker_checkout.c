...
int process_command(int cc, int num_tokens,
  ...
  case CMD_NOOP:
    if (check_number_arguments(num_tokens, 0) == OS_SUCCESS) {
      status = GENERIC_STAR_TRACKER_CommandDevice(
          &Generic_star_trackerUart, GENERIC_STAR_TRACKER_DEVICE_NOOP_CMD, 0);
  ...
  case CMD_HK:
    if (check_number_arguments(num_tokens, 0) == OS_SUCCESS) {
      status = GENERIC_STAR_TRACKER_RequestHK(&Generic_star_trackerUart,
                                              &Generic_star_trackerHK);
      if (status == OS_SUCCESS) {
  ...
  case CMD_GENERIC_STAR_TRACKER:
    if (check_number_arguments(num_tokens, 0) == OS_SUCCESS) {
      status = GENERIC_STAR_TRACKER_RequestData(&Generic_star_trackerUart,
                                                &Generic_star_trackerData);
      if (status == OS_SUCCESS) {
